/* Sorted list-array - List of elements that are sorted according to their 
weight

Christoph Reisinger, 140102- ??*/

#ifndef SORTEDLISTARRAY_HPP
#define SORTEDLISTARRAY_HPP

#include <stream.h>
#include "listarray.h"

class SortedListArray : public ListArray<int>{
...
public:
  SortedListArray(){};
  SortedListArray(const int, const int);
  int Reset(const int, const int);
  //  ~SortedListArray();

  int InsertSorted(const int, const int);
  int Tail() const;
  int CutTail();
  int Remove(const int);

  bool IsIn(const int) const;
